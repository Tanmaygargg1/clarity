...
export async function getETFHoldings(ticker) {
  ...
  // Try Yahoo Finance quote for top holdings info
  try {
    const quote = await yahooFinance.quote(ticker);
    const holdings = quote?.topHoldings?.holdings || [];
    if (holdings.length > 0) {
      const totalWeight = holdings.reduce((sum, h) => sum + (h.holdingPercent || 0), 0);
      return holdings.map((h) => ({
        ...
        totalWeight < 0.95
          ? [{ ticker: '[Other]', weight: 1 - totalWeight, name: 'Other Holdings' }]
          : []
      );
    }
  } catch (err) {
    console.error(`getETFHoldings Yahoo fallback failed for ${ticker}:`, err?.message);
  }
...
export async function decomposePortfolio(positions) {
  ...
  const quoteResults = await Promise.allSettled(
    normalizedPositions.map((p) => yahooFinance.quote(p.ticker).catch(() => null))
  );
  ...
  if (unknownTickers.length > 0) {
    const extraQuotes = await Promise.allSettled(
      unknownTickers.map((t) => yahooFinance.quote(t).catch(() => null))
    );
