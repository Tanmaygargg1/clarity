...
export function generateVerdict(xray, risk, factors, regime, behavioral, positions) {
  ...
  for (const flag of (behavioral?.behavioral_flags ?? [])) {
    if (flag === 'HOME_BIAS') {
      ...
      const k = (behavioral?.kelly ?? []).find(x => x.ticker === ticker);
      if (k) findings.push(`${ticker} is sized at ${k.ratio.toFixed(1)}x your Kelly-optimal weight given its volatility. This is a statistically dangerous overbet.`);
    }
  }
  ...
  const actions = [];
  const overbets = (behavioral?.kelly ?? []).filter(k => k.flag === 'OVERBET');
  if (overbets.length > 0) {
    const ob = overbets[0];
    const target = Math.min(ob.kelly_fraction * 1.5, ob.actual_weight * 0.5);
    const freed = ob.actual_weight - target;
    actions.push(`Reduce ${ob.ticker} from ${(ob.actual_weight * 100).toFixed(0)}% to ${(target * 100).toFixed(0)}% and redirect the ${(freed * 100).toFixed(0)}% into VXUS to reduce both concentration and home bias simultaneously.`);
  }
